...
describe 'Movies' do
  ...
  before do
    @movie = Movie.create!(title: "Birdman", mpaa_rating: 'r', total_gross: 9_300_000, description: "birdman", release_date: Date.parse("October 17, 2014"))
  end

  it "#index" do
    # Bunch of random movies to check the index action
    1.upto(5) do |i|
      Movie.create(title: "Movie_#{i}", mpaa_rating: 'r', description: "Movie_#{i} description");
    end
  ...
  def check_remote_movie(movie)
    ...
    expect(movie['mpaa_rating']).to eq "r"
    expect(movie['release_date']).to eq  Date.parse("October 17, 2014").to_s
    expect(movie['title']).to eq "Birdman"
  end
